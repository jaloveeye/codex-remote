...
let lastRelayStoreLabel = null;
/** 연결 정보 Webview용 HTML 생성 */
function getConnectionsViewHtml(data) {
    const { serverRunning, serverPort, relaySessionId, relayStoreLabel, relayServerUrl, localClientIds, codexCliStatus, extensionVersion, extensionBuildInfo, } = data;
    const relayStoreLine = relayStoreLabel != null
        ? `<p class="relay-meta"><strong>저장소:</strong> ${escapeHtml(relayStoreLabel)}</p>`
        : "";
    const relayUrlLine = relayServerUrl != null
    ...
    const codexStatusClass = codexCliStatus.severity === "error"
        ? "error"
    ...
    <section class="section">
      <h2>🤖 Codex CLI</h2>
      <p class="status ${codexStatusClass}">${escapeHtml(codexCliStatus.summary)}</p>
      <p class="relay-meta"><strong>명령:</strong> <code>${escapeHtml(codexCliStatus.command)}</code></p>
      ${codexCheckedAtLine}
    </section>`;
...
<body>
  ...
  <section class="section">
    <h2>ℹ️ 버전</h2>
    <p class="relay-meta"><strong>확장:</strong> <code>${escapeHtml(extensionVersion)}</code> · <strong>빌드:</strong> <code>${escapeHtml(extensionBuildInfo)}</code></p>
  </section>
  <section class="section">
    <h2>🔌 서버</h2>
...
function resolveExtensionBuildInfo(context) {
    try {
        const extensionJsPath = path.join(context.extension.extensionPath, "out", "extension.js");
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeConfiguration((event) => {
        ...
        outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] 🔄 Relay Server URL setting updated: ${relayServerUrl}`);
        outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] ℹ️ Reload the VS Code window to use the new relay server.`);
        updateConnectionsView();
        vscode.window.showInformationMessage("Codex Remote: relayServerUrl changed. Reload the VS Code window to reconnect with the new relay server.");
    }));
            ...
            if (relayClient && relayClient.isConnectedToSession()) {
                relayClient
                    ...
                    if (!result.success && result.error) {
                        outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] [Relay] ⚠️ 자동 연결 해제 실패: ${result.error}`);
                    }
                    if (statusBarManager) {
                    ...
                    outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] [Relay] ⚠️ 자동 연결 해제 오류: ${errorMsg}`);
                });
            }
            // 연결 상태 전송
            if (wsServer) {
    ...
    // Register commands
    const openGuideCommand = vscode.commands.registerCommand("codexRemote.openGuide", async () => {
        try {
            ...
            const errorMsg = error instanceof Error ? error.message : "Unknown error";
            vscode.window.showErrorMessage(`Codex Remote: 가이드를 열 수 없습니다 - ${errorMsg}`);
        }
    });
    const showOutputCommand = vscode.commands.registerCommand("codexRemote.showOutput", () => {
        outputChannel.show(true);
    });
    const refreshCodexStatusCommand = vscode.commands.registerCommand("codexRemote.refreshCodexStatus", () => {
    ...
    const startCommand = vscode.commands.registerCommand("codexRemote.start", () => {
        if (wsServer && !wsServer.isRunning()) {
            wsServer
                ...
                updateConnectionsView();
                vscode.window.showInformationMessage(`Codex Remote server started on port ${config_1.CONFIG.WEBSOCKET_PORT}`);
            })
                .catch((error) => {
                const errorMsg = error instanceof Error ? error.message : "Unknown error";
                outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] ❌ Failed to start WebSocket server: ${errorMsg}`);
                vscode.window.showErrorMessage(`Codex Remote: Server start failed - ${errorMsg}`);
                if (statusBarManager) {
                    statusBarManager.update(false);
                }
                updateConnectionsView();
            ...
            vscode.window.showInformationMessage("Codex Remote server is already running");
        }
    });
    const stopCommand = vscode.commands.registerCommand("codexRemote.stop", async () => {
        if (relayClient && relayClient.isConnectedToSession()) {
        ...
        if (wsServer && wsServer.isRunning()) {
            ...
            vscode.window.showInformationMessage("Codex Remote server is not running");
        }
    });
    const toggleCommand = vscode.commands.registerCommand("codexRemote.toggle", async () => {
        if (wsServer) {
    ...
    const checkRelayServerCommand = vscode.commands.registerCommand("codexRemote.checkRelayServer", async () => {
        if (relayClient) {
    ...
    const connectToRelaySessionByIdCommand = vscode.commands.registerCommand("codexRemote.connectToRelaySessionById", async () => {
        if (!relayClient) {
            outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] [Relay] ⚠️ Relay client not initialized`);
            outputChannel.show();
            return;
    ...
    const setRelaySessionIdCommand = vscode.commands.registerCommand("codexRemote.setRelaySessionId", async () => {
        const sid = await vscode.window.showInputBox({
            title: "Codex Remote: 릴레이 세션 ID 설정",
            prompt: "다음 릴레이 시작 시 사용할 세션 ID 6자 (모바일에서 같은 ID로 연결)",
            placeHolder: "3ZUESK",
            value: context.globalState.get("codexRemote.sessionId") ?? "",
            validateInput: (value) => {
                const v = (value ?? "").trim().toUpperCase();
                if (!v)
                    return "세션 ID를 입력하세요.";
        ...
        if (sid) {
            await context.globalState.update("codexRemote.sessionId", sid.trim().toUpperCase());
            vscode.window.showInformationMessage(`Codex Remote: 세션 ID가 ${sid
                .trim()
                .toUpperCase()}로 저장되었습니다. (다음 릴레이 시작 시 사용)`);
        }
    });
    const showConnectionsCommand = vscode.commands.registerCommand("codexRemote.showConnections", async () => {
        if (connectionsPanel) {
    ...
    const statusBarClickCommand = vscode.commands.registerCommand("codexRemote.statusBarClick", async () => {
        ...
        try {
            ...
            if (statusBarManager)
                statusBarManager.refresh();
            updateConnectionsView();
            vscode.window.showInformationMessage(`Codex Remote: 세션 ${sidTrimmed}에 연결되었습니다.`);
        }
        catch (error) {
            const errorMsg = error instanceof Error ? error.message : "Unknown error";
    ...
    relayClient = new relay_client_1.RelayClient(getRelayServerUrl(), outputChannel);
    ...
    wsServer
        ...
        updateConnectionsView();
        outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] [Relay] 릴레이 비활성. 상태줄 'Codex Remote' 클릭 → 세션 ID·PIN 입력하여 연결`);
    })
        .catch((error) => {
        const errorMsg = error instanceof Error ? error.message : "Unknown error";
        outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] ❌ Failed to start WebSocket server: ${errorMsg}`);
