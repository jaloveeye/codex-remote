...
export class WebSocketServer {
  ...
  private readonly relayForwardTypeAllowlist = new Set<string>([
    "chat_response",
    "chat_response_chunk",
  ...
  async start(): Promise<void> {
    ...
    if (availablePort === null) {
      ...
      vscode.window.showErrorMessage(`Codex Remote: ${errorMsg}`);
    ...
    if (availablePort !== this.port) {
      ...
      vscode.window.showWarningMessage(
        `Codex Remote: 포트 ${this.port}가 사용 중이어서 포트 ${availablePort}로 시작합니다.`
      );
    }
    ...
    return new Promise((resolve, reject) => {
      this.wss!.on("connection", (ws: WebSocketClient) => {
        ...
        this.clients.add(ws);
        this.log(`Client connected to Codex Remote (ID: ${clientId})`);
        ...
        ws.on("close", () => {
          ...
          this.log(
            `Client disconnected from Codex Remote (ID: ${disconnectedClientId})`
          );
        ...
        this.sendToClient(
          ...
          JSON.stringify({
            type: "connected",
            message: "Connected to Codex Remote",
          })
        );
      });
      ...
      this.wss!.on("error", (error: any) => {
        ...
        if (error.code === "EADDRINUSE") {
          const errorMsg = `포트 ${availablePort}가 사용 중입니다. 다른 프로세스를 종료하거나 VS Code를 재시작해주세요.`;
          vscode.window.showErrorMessage(`Codex Remote: ${errorMsg}`);
          reject(new Error(errorMsg));
        } else {
          vscode.window.showErrorMessage(
            `Codex Remote server error: ${error.message}`
          );
          reject(error);
        }
      });
