...
export class RelayClient {
  ...
  private readonly POLL_IDLE_INTERVAL = 1000; // 유휴 상태 1초 폴링
  private readonly POLL_ACTIVE_INTERVAL = 250; // 활성 상태 0.25초 폴링
  ...
  private disconnectInFlight: Promise<{ success: boolean; error?: string }> | null =
    null;
  /** poll 중복 실행 방지 */
  ...
  private startPolling(): void {
    ...
    this.pollInterval = setInterval(() => {
      if (!this.isRunning || pollingRunToken !== this.runToken) return;
      const now = Date.now();
          ...
          const payload =
            ...
            !Array.isArray(basePayload)
              ? {
                  ...basePayload,
                  senderDeviceId:
                    (basePayload as { senderDeviceId?: string }).senderDeviceId ??
                    msg.senderDeviceId,
                  targetDeviceId:
                    (basePayload as { targetDeviceId?: string }).targetDeviceId ??
                    msg.targetDeviceId,
                }
              : basePayload;
          const messageStr =
  ...
  private async discoverSession(): Promise<string | null> {
    ...
    try {
      ...
      if (sessionsCount === 0) {
        ...
        this.log(
          "💡 다른 VS Code 창이 열려 있으면 그 익스텐션이 세션을 먼저 가져갔을 수 있습니다. 다른 창을 모두 닫고 새 세션으로 다시 시도해 보세요."
        );
        const debugUrl = `${this.relayServerUrl}/api/debug-sessions`;
        this.log(
          `🔧 서버 상태 확인: GET ${debugUrl} (또는 명령 팔레트에서 "Codex Remote: 릴레이 서버 상태 확인" 실행)`
        );
        return null;
      }
   ...
   */
  async disconnectCurrentSession(): Promise<{ success: boolean; error?: string }> {
    ...
    this.disconnectInFlight = (async () => {
      ...
      try {
        const result = await this.httpRequestWithStatus(
          `${this.relayServerUrl}/api/disconnect`,
          "POST",
          {
        ...
        if (result.statusCode === 404) {
          ...
          this.log(
            `ℹ️ 릴레이 세션 ${currentSessionId}는 서버에 이미 없음(로컬 연결 정리 진행)`
          );
          return { success: true };
        }
